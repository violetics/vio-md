...
async function WAVideo(file, ext1) {
	return convert(file, ext1, "mp4", ["-c:a aac", "-c:v libx264", "-b:a 128K", "-ar 44100", "-crf 28", "-preset slow"]);
}
...
function stickerWithExif(file, ext, packInfo, cmd) {
	return new Bluebird(async (res, rej) => {
		...
		ex.create(packname !== undefined || "" ? packname : "Original", author !== undefined || "" ? author : "Rzky-Bot", filename);
		...
		run(`webpmux -set exif ./temp/${filename}.exif ./temp/${filename}.webp -o ./temp/${filename}.webp`, async (err) => {
			if (err) rej(err) && (await Promise.all([fs.unlink(`./temp/${filename}.webp`), fs.unlink(`./temp/${filename}.exif`)]));
			setTimeout(() => {
