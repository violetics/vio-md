...
async function serialize(msg, conn) {
	...
	conn.sendFile = async (jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) => {
		...
		else if (/audio/.test(type.mime)) throw new Error("Unsupported MimeType <audio>");
		else mtype = "document";
	...
	if (msg.message) {
		...
		msg.mentions = Object.keys(msg.message[msg.type] || {}).includes("contextInfo") ? msg.message[msg.type].contextInfo.mentionedJid : [];
		try {
			...
			msg.quoted.id = msg.quoted.stanzaId;
			msg.quoted.idLength = msg.quoted.id.length;
			msg.quoted.isBaileys = msg.quoted.id.startsWith("BAE") && msg.quoted.id.length == 16;
			...
			msg.quoted.text =
				...
				(msg.quoted.mtype == "stickerMessage" && Buffer.from(msg.quoted.message[msg.quoted.mtype].fileSha256, "base64").toString("hex")) ||
		...
		msg.body =
			...
			(msg.type === "stickerMessage" && Buffer.from(msg.message[msg.type].fileSha256, "base64").toString("hex")) ||
		...
		msg.adReply = (text, options = {}) => {
			return conn.sendMessage(
				msg.from,
				{
					...
					contextInfo: {
						externalAdReply: {
							title: `©${new Date().getFullYear()} ${config.name}`,
							body: "Vio-MD is multi-device whatsapp bot using library @adiwajshing/baileys and example bot of Violetics API",
							mediaType: 3,
							renderLargerThumbnail: true,
							showAdAttribution: true,
							sourceUrl: `${config.BASEURL}/`,
							mediaUrl: `${config.BASEURL}/`,
							thumbnailUrl: `${config.BASEURL}/`,
							thumbnail: fs.readFileSync("./tmp/violetics.png"),
						},
					},
				},
				{ quoted: msg, ...options }
		...
		msg.reply = (text, options = {}) => conn.sendMessage(msg.from, { text: util.format(text) }, { quoted: msg, ...options });
		msg.sendImage = (image, caption = "", options = {}) =>
			conn.sendMessage(
				msg.from,
				{ image: Buffer.isBuffer(image) ? Buffer.from(image) : { url: image }, caption: util.format(caption), jpegThumbnail: fs.readFileSync("./tmp/violetics.png") },
				{ quoted: msg, ...options }
			);
