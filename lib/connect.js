	...
	try {
		for (var dir of dirs) {
			...
			for (let file of cmds) {
				...
				let options = {};
				for (var k in cmd) !["name", "alias", "description", "category", "exec"].includes(k.toLowerCase()) ? (options[k] = cmd[k]) : "";
				let cmdObject = {
					name: cmd.name,
					alias: cmd.alias,
					description: cmd.description,
					category: groups,
					params: Array.isArray(cmd.params) ? cmd.params.map((x) => `<${x}>`) : [`<${cmd.params}>`],
					options: options,
					exec: cmd.exec,
					path: path,
				};
