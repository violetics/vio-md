...
class Request {
	...
	send(options = {}) {
		...
		const APIKEY = (this.options && this.options.params && this.options.params.apikey) ? this.options.params.apikey : this.APIKEY;
		...
		const URL_PATH = this.PATH.startsWith("/api") ? this.BASEURL_PATH.concat(`?apikey=${APIKEY}`) : this.BASEURL_PATH;
		return axios({
			url: URL_PATH,
			...this.options,
			headers: {
